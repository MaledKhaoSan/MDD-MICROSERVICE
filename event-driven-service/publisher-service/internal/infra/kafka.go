// publisher-service/internal/infra/kafka.go
package infra
...
func InitKafkaProducer() {
	...
	config.Producer.Return.Successes = true          // รอผลลัพธ์เมื่อส่งสำเร็จ
	config.Producer.Retry.Max = 5                    // ลองใหม่สูงสุด 5 ครั้ง
	config.Producer.Retry.Backoff = 100 * time.Millisecond // หน่วงเวลา retry
...
func PublishToKafka(topic string, payload []byte) error {
	msg := &sarama.ProducerMessage{
		Topic: topic,
		Value: sarama.ByteEncoder(payload), // ใช้ ByteEncoder เพื่อส่ง payload โดยตรง
	}
