...
import (
	...
	"github.com/MD-PROJECT/INVENTORY-DETAIL-SERVICE/internal/event"
...
func StartKafkaConsumer(db *gorm.DB) { // Added db parameter
	config := &kafka.ConfigMap{
		...
		"group.id":          "inventory-subscribe-service-group",
...
var topicHandlers = map[string][]EventHandler{
	"InventoryInBoundQuantityEvent": {
		event.InventoryInBoundQuantityEvent,
	},
	"InventoryOutBoundQuantityEvent": {
		event.InventoryOutBoundQuantityEvent,
	},
	"OrderCreatedEvent": {
		event.StockDecreaseOrderCreatedEvent,
	},
	"ProductCreatedEvent": {
